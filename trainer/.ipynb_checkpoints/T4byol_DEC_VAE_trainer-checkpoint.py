...
class BYOLTrainer():
    ...
    def construct_model(self):
        ...
        self.codebook = VectorQuantizerEMA(num_embeddings=1024, embedding_dim=256, commitment_cost=0.25, decay=0.99)
        self.codebook = self.codebook.to(self.device)
        
        self._pre_CB_conv = nn.Conv2d(in_channels=256, 
                                      out_channels=256,
                                      kernel_size=1, 
                                      stride=1)
        self._pre_CB_conv = self._pre_CB_conv.to(self.device)
        ...
        params = params_util.collect_params([self.model.online_network, self.model.predictor,
                                            self._pre_CB_conv,self.codebook,self.vae_projection],
...
#             _, enc_quantize, _, _ = self.vae._vq_vae(enc)
            ...
            q_map1 = self._pre_CB_conv(q_map1)
            _, quantized1, _, _ = self.codebook(q_map1)
            z_recon1 = self.vae._decoder(quantized1)
            
            
            q_map2 = self._pre_CB_conv(q_map2)
            _, quantized2, _, _ = self.codebook(q_map2)
            z_recon2 = self.vae._decoder(quantized2)
