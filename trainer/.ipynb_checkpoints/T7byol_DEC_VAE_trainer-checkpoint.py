...
#         self.vae.load_state_dict(vae_checkpoint['model'])
    ...
    def train_epoch(self, epoch):
        ...
        while images is not None:
            ...
            x_recon,quantized,vae_loss, perplexity = self.vae(vae_image)
            embd = self.vae_projection(quantized)
            
            qq_map1 = self.vae._vq_vae(q_map1)
            qq_map2 = self.vae._vq_vae(q_map2)
            qq_map1 = self.vae._pre_vq_conv(qq_map1)
            qq_map2 = self.vae._pre_vq_conv(qq_map2)

            
            x_recon1 = self.vae._decoder(qq_map1)
            x_recon2 = self.vae._decoder(qq_map2)
            ...
            loss5 = self.mse_criterion(x_recon1,vae_image)
            loss6 = self.mse_criterion(x_recon2,vae_image)
            
#             loss5 = self.forward_loss(target_z1, embd)
#             loss6 = self.forward_loss(target_z2, embd)
        
            
            NC_loss = (loss1+loss2+loss3+loss4)/6
            MSE_loss = vae_loss + (loss5+loss6)/2
