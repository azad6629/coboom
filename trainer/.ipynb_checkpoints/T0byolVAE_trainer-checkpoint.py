...
class BYOLTrainer():
    def __init__(self, config):
        ...
        self.enc_ckpt_path = '/workspace/mayankk/Radiomics/BERT-BYOL/trainer/mimic_encoder.pth'
...
#         net_name = self.config['model']['backbone']['type']
#         base_encoder = models.__dict__[net_name](pretrained=False)
        base_encoder = models.resnet50(pretrained=False)
        base_encoder.fc = nn.Identity()
        enc_checkpoint = torch.load(self.enc_ckpt_path, map_location=self.device)['encoder']

        state_dict = {}
        length = len(base_encoder.state_dict())
        for name, param in zip(base_encoder.state_dict(), list(enc_checkpoint.values())[:length]):
            state_dict[name] = param
        base_encoder.load_state_dict(state_dict)
        
#         self.encoder = nn.Sequential(*list(base_encoder.children())[:-1])
        self.extra_encoder = base_encoder.to(self.device)
        self.extra_projection = MLP(2048,1024,256).to(self.device)
...
#                               momentum=momentum, weight_decay=weight_decay)
    ...
    def train_epoch(self, epoch):
        ...
        while images is not None:
            ...
            enc = self.extra_encoder(vae_image)
            
            embd = self.extra_projection(enc)
...
#             loss4 = self.forward_loss(q2, embd)
            mse_loss1 = self.mse(q1,embd)
            mse_loss2 = self.mse(q2,embd)
            sim_loss = (mse_loss1 + mse_loss2)/2
            
#             loss5 = self.forward_loss(target_z1, embd)
#             loss6 = self.forward_loss(target_z2, embd)
            
            nc_loss = (loss1+loss2)/2
            
            loss = sim_loss +nc_loss
